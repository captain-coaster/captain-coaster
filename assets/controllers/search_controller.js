...
import { SEARCH_INDEX_MORE, SEARCH_INDEX_NO_RESULT, APP_SEARCH_SEARCHING, trans } from '../translator';
...
export default class extends Controller {
    ...
    setupAccessibility() {
        ...
        if (this.hasDropdownTarget) {
            ...
            this.dropdownTarget.setAttribute('aria-label', 'Search suggestions');
        }
    }
     ...
     */
     updateResults(data) {
        ...
        if (results.coasters) {
            allResults = allResults.concat(results.coasters.map(item => ({...item, emoji: '🎢'})));
        }
        if (results.parks) {
            allResults = allResults.concat(results.parks.map(item => ({...item, emoji: '🎡'})));
        }
        if (results.users) {
            allResults = allResults.concat(results.users.map(item => ({...item, emoji: '👤'})));
        }
    ...
    renderResultItem(item, index, query) {
        ...
        let subtitle = item.subtitle ? this.highlightSearchTerm(item.subtitle, query) : null;
        ...
        if (item.type === 'park' && subtitle && subtitle.includes('country.')) {
            ...
            if (countryMatch) {
                const translatedCountry = this.translateCountry(countryMatch[0]);
                subtitle = subtitle.replace(/<strong>.*?<\/strong>|country\.\w+/g, (match) => {
                    return match.startsWith('<strong>') ? match : translatedCountry;
                });
            }
        }
    ...
    translateCountry(countryKey) {
        ...
        return countryKey.replace('country.', '').replace(/^\w/, c => c.toUpperCase());
    }
    ...
    handleKeydown(event) {
        ...
        const items = this.resultsTarget.querySelectorAll('.search-result-item, .search-show-more');
    ...
    setupResultClickHandlers() {
        const resultItems = this.resultsTarget.querySelectorAll('.search-result-item');
    ...
    generateRoute(routeName, params = {}) {
        if (typeof Routing !== 'undefined' && Routing.generate) {
            try {
                return Routing.generate(routeName, { ...params, '_locale': document.documentElement.lang || 'en' });
            } catch (error) {
    ...
    updateClearButtonVisibility() {
        const inputContainer = this.element.querySelector('.search-input-container');
        if (inputContainer && this.hasInputTarget) {
            inputContainer.classList.toggle('has-content', this.inputTarget.value.trim().length > 0);
        }
    }
