...
export default class extends Controller {
            ...
            switch (event.key) {
                ...
                case 'Enter':
                    ...
                    if (this.selectedIndex >= 0 && items[this.selectedIndex]) {
                        this.selectResult({ target: items[this.selectedIndex] });
                    }
                    break;
    ...
    selectResult(event) {
        ...
        if (!type || !slug) {
            console.error('Missing required data attributes for result selection');
            return;
        }
    ...
    updateSelection(items) {
        ...
        if (this.selectedIndex >= 0 && items[this.selectedIndex]) {
            items[this.selectedIndex].classList.add('search-result-item-keyboard-selected');
     ...
     */
    clearSelection() {
        const selectedItems = this.element.querySelectorAll('.search-result-item-keyboard-selected');
        selectedItems.forEach(item => item.classList.remove('search-result-item-keyboard-selected'));
        this.selectedIndex = -1;
    }
    ...
    generateRoute(type, params = {}) {
        ...
        if (typeof Routing !== 'undefined' && Routing.generate) {
            try {
                ...
                console.error('Routing.generate failed for route:', routeName, 'with params:', params, error);
                return null;
            }
        }
        
        // If Routing is not available, log error and return null
        console.error('FOSJsRoutingBundle not available. Cannot generate route for type:', type);
        return null;
    }
    ...
    resultItemTargetDisconnected(element) {
        ...
        element.classList.remove('search-result-item-hover', 'search-result-item-selected', 'search-result-item-keyboard-selected');
    }
}