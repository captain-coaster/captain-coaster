...
`feature/tailwind` is an in-progress migration of the frontend to Tailwind CSS v4 + Vite + Symfony UX Twig/Live Components, with its own more detailed `CLAUDE.md`. Once that branch merges, replace this file with its version rather than reconciling the two by hand.
