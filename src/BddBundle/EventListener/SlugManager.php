...
namespace BddBundle\EventListener;

use BddBundle\Entity\Coaster;
use Doctrine\Common\EventSubscriber;
use Doctrine\Common\Persistence\Event\LifecycleEventArgs;

class SlugManager implements EventSubscriber
{
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        ...
        if (!$object instanceof Coaster) {
            return;
        }

        $slug = $this->slugify(sprintf('%s-%s', $object->getName(), $object->getPark()->getName()));
        $object->setSlug($slug);
    }
     ...
     * @return mixed|string
     */
    private function slugify(string $input, array $options = [])
    {
        // Make sure string is in UTF-8 and strip invalid UTF-8 characters
        $input = mb_convert_encoding((string)$input, 'UTF-8', mb_list_encodings());
        ...
        if ($options['transliterate']) {
            $input = str_replace(array_keys($char_map), $char_map, $input);
        }
