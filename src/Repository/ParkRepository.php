...
class ParkRepository extends ServiceEntityRepository
{
    ...
    /**
     * Returns the closest parks with their best-ranked coaster's main image filename.
     *
    ...
    public function getClosestParks(Park $park, int $minScore, int $maxDistance): array
    {
        ...
        // Fetch nearby parks with distance — GROUP BY p.id to avoid duplicates
        // when a park has multiple qualifying coasters
        ...
        $rows = $this->getEntityManager()
            ...
            ->select(
                ...
                'p.name as name',
                'p.slug as slug',
                'ROUND(( 6371 * acos( cos( radians(:parkLatitude) )
        ...
        // Fetch best-ranked coaster image for each nearby park in one query
        $parkIds = array_column($rows, 'id');
        /** @var array<int, array{parkId: mixed, imageFilename: mixed}> $bestImages */
        $bestImages = $this->getEntityManager()
            ->createQueryBuilder()
        ...
        // Index by parkId — first result per park is the best-ranked
        /** @var array<int, string> $imageByPark */
        $imageByPark = [];
        foreach ($bestImages as $imageRow) {
            $pid = (int) $imageRow['parkId'];
            if (0 !== $pid && !isset($imageByPark[$pid])) {
                $imageByPark[$pid] = (string) $imageRow['imageFilename'];
            }
        }

        foreach ($rows as &$row) {
            $row['bestCoasterImage'] = $imageByPark[(int) $row['id']] ?? null;
        }
