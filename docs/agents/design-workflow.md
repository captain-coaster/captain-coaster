...
- **Preload only the body face's latin subset**, with `crossorigin` [web.dev:fonts] (preload is widely available [WS:preload]). Vite rewrites and hashes CSS `url()` fonts, and inlines files under `assetsInlineLimit` [Vite:assets]. The reprise Twig helpers only expose entry files, not individual assets (`vendor/symfony/reprise/src/Twig/AssetExtension.php:30-34`). So the preload needs a stable URL. Decided: serve the woff2 files from `public/fonts/` with the version in the filename, a long cache time, and `asset()` preload in `base.html.twig`.
